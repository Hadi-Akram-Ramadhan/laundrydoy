...
if(isset($_POST['btn-simpan'])){
    ...
    // Tambah pengecekan no_ktp duplikat
    $check_ktp = "SELECT COUNT(*) as count FROM member WHERE no_ktp = '$no_ktp'";
    $result = mysqli_query($conn, $check_ktp);
    $data = mysqli_fetch_assoc($result);
    
    if($data['count'] > 0) {
        echo "<script>
            alert('No KTP sudah terdaftar! Silakan gunakan No KTP lain.');
            window.history.back();
        </script>";
        exit();
    }
