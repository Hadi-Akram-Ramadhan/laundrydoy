...
if(isset($_POST['btn-simpan'])){
    ...
    // Check if KTP already exists for different member
    $check_ktp = "SELECT * FROM member WHERE no_ktp = '$no_ktp' AND id_member != '$id_member'";
    $result = mysqli_query($conn, $check_ktp);
    
    if(mysqli_num_rows($result) > 0) {
        echo "<script>alert('No KTP sudah terdaftar! Gunakan no KTP yang lain.');window.history.back();</script>";
        exit;
    }
